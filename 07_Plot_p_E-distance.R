####################################################
###    PLOT PROBABILITIES FOR E-DISTANCES        ###
####################################################
...
for(q in c("T", "X", "S", "M", "K", "L"))
{
  if(q %in% c("T", "X", "S", "M"))
  {
    dat1 <- STAT.NULL3.2[[q]] %>% filter(ID %in% unique(.$ID[.$INVASION.STATUS == "native"]) & ID %in% unique(.$ID[.$INVASION.STATUS == "naturalized"])) %>% mutate(INVASION.STATUS = as.character(INVASION.STATUS)) %>% 
      filter(INVASION.STATUS %in% c("naturalized")) %>% mutate(comparison = "natz.plots")
    dat2 <- STAT.NULL3.2[[q]] %>% filter(ID %in% unique(.$ID[.$INVASION.STATUS == "native"]) & ID %in% unique(.$ID[.$INVASION.STATUS == "invasive"])) %>% mutate(INVASION.STATUS = as.character(INVASION.STATUS)) %>%
      filter(INVASION.STATUS %in% c("invasive")) %>% mutate(comparison = "inv.plots")
    dat3 <- STATali.NULL3.2[[q]] %>% filter(ID %in% unique(.$ID[.$INVASION.STATUS == "naturalized"]) & ID %in% unique(.$ID[.$INVASION.STATUS == "invasive"])) %>% mutate(INVASION.STATUS = as.character(INVASION.STATUS)) %>%
      filter(INVASION.STATUS %in% c("invasive")) %>% mutate(comparison = "ali.plots")
    
    dat_long <- rbind(dat1[, colnames(dat3)], dat2[, colnames(dat3)], dat3)
    head(dat_long)
    ...
    gg.plots[[q]] <- ggplot(dat_long, aes(x = comparison, y = `Edist_P-val`, fill = INVASION.STATUS)) +
      ...
      geom_boxplot_pattern(pattern = 'stripe', 
                           pattern_fill = my.cols2[c(1,1,2)], 
                           pattern_colour = my.cols2[c(2,2,3)], 
                           colour = "black",
  ...
  if(q %in% c("K", "L") )
  {
    ...
    {
      dat1 <- STAT.NULL3.2[[q]][[m]] %>% filter(ID %in% unique(.$ID[.$INVASION.STATUS == "native"]) & ID %in% unique(.$ID[.$INVASION.STATUS == "naturalized"])) %>% mutate(INVASION.STATUS = as.character(INVASION.STATUS)) %>% 
        filter(INVASION.STATUS %in% c("naturalized")) %>% mutate(comparison = "natz.plots")
      dat2 <- STAT.NULL3.2[[q]][[m]] %>% filter(ID %in% unique(.$ID[.$INVASION.STATUS == "native"]) & ID %in% unique(.$ID[.$INVASION.STATUS == "invasive"])) %>% mutate(INVASION.STATUS = as.character(INVASION.STATUS)) %>%
        filter(INVASION.STATUS %in% c("invasive")) %>% mutate(comparison = "inv.plots")
      dat3 <- STATali.NULL3.2[[q]][[m]] %>% filter(ID %in% unique(.$ID[.$INVASION.STATUS == "naturalized"]) & ID %in% unique(.$ID[.$INVASION.STATUS == "invasive"])) %>% mutate(INVASION.STATUS = as.character(INVASION.STATUS)) %>%
        filter(INVASION.STATUS %in% c("invasive")) %>% mutate(comparison = "ali.plots")
...
tiff("Fig3.tif", width = 11.5, height = 16, units = "in", compression = "lzw", res = 500)
