...
###DATASET WITHOUT PROMINENT INVADERS---------------------------------------------------
##Remove 10% of prominent invaders

library(tidyverse)

spel.imp.ls.out <- spel.imp.ls
...
for(q in c("T", "X", "S", "M", "K", "L"))
{
  ...
  if(q %in% c("T", "X", "S", "M"))
  {
    ...
    out <- which(spel.imp.ls.out$Habitat == q &
                   spel.imp.ls.out$INVASION.STATUS == "naturalized" &
                   spel.imp.ls.out$Species %in% names(sel))
    
    spel.imp.ls.out <- spel.imp.ls.out[-out,]
    ...
    out <- which(spel.imp.ls.out$Habitat == q &
                   spel.imp.ls.out$INVASION.STATUS == "invasive" &
                   spel.imp.ls.out$Species %in% names(sel))
