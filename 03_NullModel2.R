###NULL MODEL 2-----------------------------------------------------------------
##Simulates virtual communities by the weighted drawing of species from the 
#native/naturalized species pool.
#Species frequency in the habitat is used as a weight. The higher the frequency, 
...
source("./Functions/null2.simulator.R")
source("./Functions/null2ali.simulator.R")
...
NULL2.missFor <- list(T=list(), X=list(), S=list(), M=list(), K=list(), L=list())
...
for(q in c("T", "X", "S", "M", "K", "L"))
{
  ...
  if(q %in% c("T", "X", "S", "M"))
  {
    ...
    cl <- makeCluster(10)
    clusterExport(cl, list("null2.simulator", "as.randtest", "stats", "stats.calc", "%>%", "column_to_rownames", "mutate", "rownames_to_column", "remove_rownames", "Edistance_weighted"))
    clusterSetRNGStream(cl,1234)

    d <- pblapply(l, FUN = null2.simulator, cl = cl,
                  ...
                  freq = SpecData.missFor[SpecData.missFor$NATIVE.ALIEN == "native" & SpecData.missFor[, q] > 0, q],
                  tr = SpecData.missFor, tr.vars = tr.vars, nperm = 999,
                  save.simulations = NULL)

    stopCluster(cl)

    NULL2.missFor[[q]] <- do.call(rbind.data.frame, d)
  ...
  if(q %in% c("K", "L"))
  {
    ...
    for(m in c("Herb", "Tree"))
    {
      ...
      d <- pblapply(l, FUN = null2.simulator, cl = cl,
                    ...
                    freq = SpecData.missFor[SpecData.missFor$NATIVE.ALIEN == "native" & SpecData.missFor$Herb.Tree == m & SpecData.missFor[, q] > 0, q],
                    tr = SpecData.missFor, tr.vars = tr.vars, nperm = 999,
                    save.simulations = NULL)
...
saveRDS(NULL2.missFor, file="NULL2.missFor.rds")

###Run simulation for naturalized vs invasive species--------------------------- 

NULL2ali.missFor <- list(T=list(), X=list(), S=list(), M=list(), K=list(), L=list())
...
for(q in c("T", "X", "S", "M", "K", "L"))
{
  ...
  if(q %in% c("T", "X", "S", "M"))
  {
    ...
    cl <- makeCluster(10)
    clusterExport(cl, list("null2ali.simulator", "as.randtest", "stats.ali", "stats.ali.calc", "%>%", "column_to_rownames", "mutate", "rownames_to_column", "remove_rownames", "Edistance_weighted"))
    clusterSetRNGStream(cl,1234)
    
    d <- pblapply(l, FUN = null2ali.simulator, cl = cl, 
                  ...
                  freq.natv = SpecData.missFor[SpecData.missFor$NATIVE.ALIEN == "native" & SpecData.missFor[, q] > 0, q],
                  ...
                  freq.natz = SpecData.missFor[SpecData.missFor$INVASION.STATUS == "naturalized" & SpecData.missFor[, q] > 0, q],
                  tr = SpecData.missFor, tr.vars = tr.vars, nperm = 999,
                  save.simulations = NULL)
    
    stopCluster(cl)
    
    NULL2ali.missFor[[q]] <- do.call(rbind.data.frame, d)
    NULL2ali.missFor[[q]] <- NULL2ali.missFor[[q]][ NULL2ali.missFor[[q]]$INVASION.STATUS != "native", ]
  ...
  if(q %in% c("K", "L"))
  {
    ...
    for(m in c("Herb", "Tree"))
    {
      ...
      d <- pblapply(l, FUN = null2ali.simulator, cl = cl, 
                    ...
                    freq.natv = SpecData.missFor[SpecData.missFor$NATIVE.ALIEN == "native" & SpecData.missFor$Herb.Tree == m & SpecData.missFor[, q] > 0, q],
                    ...
                    freq.natz = SpecData.missFor[SpecData.missFor$INVASION.STATUS == "naturalized" & SpecData.missFor$Herb.Tree == m & SpecData.missFor[, q] > 0, q],
                    tr = SpecData.missFor, tr.vars = tr.vars, nperm = 999,
                    save.simulations = NULL)
    ...
    NULL2ali.missFor[[q]] <- ht
  }
  
}

saveRDS(NULL2ali.missFor, file="NULL2ali.missFor.rds")