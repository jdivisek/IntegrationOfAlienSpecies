##Simulates virtual communities by co-occurrence weighted drawing of species from the native species pool.
...
#@beals.scores = Matrix of occurrence probabilities for the native species in the vegetation plots returned by the vegan::beals function.
#@bl = Life forms of the species in the beals.scores matrix.
...
#@nperm = Number of communities to be simulates.
...
null3.gpcoa.simulator <- function(one.plot, beals.scores, bl, tr, tr.vars, nperm=999, save.simulations=NULL)
{
  if(length(unique(one.plot$INVASION.STATUS)) > 1 & any(one.plot$INVASION.STATUS %in% c("native")))
  {
    ...
    repeat
    {
      ...
      {
        w <- setNames(beals.scores[as.character(x$ID[1]), bl==x$Life.form[1]],
                      colnames(beals.scores)[bl==x$Life.form[1]])
        if(sum(w) > 0){
    ...
    obs <- stats.gpcoa.calc(one.plot = one.plot, tr.vars = tr.vars)
    obs.t <- apply(X = one.plot[, tr.vars], MARGIN = 2, FUN = stats.gpcoa, 
                   st = one.plot$INVASION.STATUS, cover = one.plot$cover, simplify = F)
    ...
    sim <- lapply(X = op.sim, FUN = function(x){
      si <- stats.gpcoa(dat = x[, tr.vars], st=x$INVASION.STATUS,
                        cover = x$cover)
      si.t <- apply(X = x[, tr.vars], MARGIN = 2, FUN = stats.gpcoa, st = x$INVASION.STATUS,
